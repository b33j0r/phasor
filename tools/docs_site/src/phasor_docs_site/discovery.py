    ...
    for name, relative_dir, wasm_supported_text in MANAGED_PROJECT_RE.findall(build_text):
        ...
        records.append(
            ExampleRecord(
                ...
                wasm_build_step=f"zig build build-{name}-wasm" if wasm_supported_text == "true" else None,
                wasm_run_step=f"zig build run-{name}-wasm" if wasm_supported_text == "true" else None,
                local_run_step="zig build run",
                local_wasm_build_step="zig build web" if wasm_supported_text == "true" else None,
                local_wasm_run_step="zig build run-wasm" if wasm_supported_text == "true" else None,
