...
pub fn build(b: *std.Build) void {
    ...
    const run_step = b.step("run", "Run the cube example");
...
fn addWebBuild(
    ...
    const server_exe = b.addExecutable(.{
        .name = "cube_wasm_server",
        .root_module = server_mod,
    });
    b.installArtifact(server_exe);

    const web_dir = "web/cube";
    ...
    const run_wasm = b.step("run-wasm", "Run the cube example in the browser");
