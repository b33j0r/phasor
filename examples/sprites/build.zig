...
pub fn build(b: *std.Build) void {
    ...
    const app_mod = b.createModule(.{ .root_source_file = b.path("main.zig"), .target = host_target, .optimize = optimize, .imports = &.{
        .{
            .name = "phasor",
            .module = phasor,
        },
    ...
    const run_step = b.step("run", "Run the sprites example");
...
fn addWebBuild(
    ...
    const server_exe = b.addExecutable(.{
        .name = "sprites_wasm_server",
        .root_module = server_mod,
    });
    b.installArtifact(server_exe);

    const web_dir = "web/sprites";
    ...
    const run_wasm = b.step("run-wasm", "Run the sprites example in the browser");
